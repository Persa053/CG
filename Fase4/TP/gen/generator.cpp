...
void plano(float lado, float grid, string nome) {
    ...
    for(int i=0;i<grid;i++){
        for(int j = 0;j<grid;j++){
            ...
            file << x << "," << y << "," << z << endl;
            file << 0 << "," << 1 << "," << 0 << endl;
            file << 1-j*v << "," << 1-i*v << endl;
            printf("%f %f %f\n",x,y,-z);
            file << x1 << "," << y << "," << z << endl;
            file << 0 << "," << 1 << "," << 0 << endl;
            file << 1 -v*(j + 1) << "," << 1-i*v << endl;
            printf("%f %f %f\n",-x,y,-z);
            file << x1 << "," << y << "," << z1 << endl;
            file << 0 << "," << 1 << "," << 1 << endl;
            file << 1 -v*(j + 1) << "," << 1-v*(i + 1)<< endl;
...
void cross_Gen(float *a, float *b, float *res) {
...
void box(int aresta, int grid, string nome){
    ...
    for(int i=0;i<grid;i++) {
        for (int j = 0; j < grid; j++) {
            ...
            file << 1 << "," << 0 << "," << 0 << endl;
            file << 1 -v*(j+1) << "," << 1-(i+1)*v << endl;
            printf("%f %f %f\n", x, y, -z);
            file << x1 << "," << yteto << "," << z << endl;
            file << 1 << "," << 0 << "," << 0 << endl;
            file << 1 -v*(j+1) << "," << 1-v*(i) << endl;
            ...
            file << 1 << "," << 0 << "," << 0 << endl;
            file << 1-v*(j) << "," << 1-v*(i + 1) << endl;
            printf("%f %f %f\n", x, y, z);
            file << x1 << "," << yteto << "," << z1 << endl;
            file << 1 << "," << 0 << "," << 0 << endl;
            file << 1-(j+1)*v << "," << 1-v*(i + 1) << endl;
...
void sphere(int raio, int slices, int stacks, string nome){
    ...
    float v = 1/ (double ) slices;
    float h = 1/ (double ) stacks;
...
void cone(float raio, float height, float slices, float stacks, string nome){
    ...
    for(int i = 0; i < slices; i++){
        ...
        float plot = (i %(int) t);
        float plot2 = ((i+1) %(int) t);
        float mod = 0.5 * plot;
        float mod2 = 0.5*plot2;
    ...
    for(int i = 0; i < stacks; i++){
        ...
        for(int j = 0; j < slices; j++){
            ...
            float ponto[3] = {x3,y3,z3};
            float centro [3] = {0,(esph*i)-(esph*(i+1)),0};
            float* res = vec(ponto,centro);
            normalize_Gen(res);
...
void torus(double r1, double r2, double divc, double divd, string nome){
    ...
    float c = 1/(double) divc;
    float d = 1/(double) divd;
